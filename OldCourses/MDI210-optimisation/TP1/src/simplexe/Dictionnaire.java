...
public class Dictionnaire extends Observable {
	...
	public boolean estRealisable() {
		...
		if(flag==true)
			Simplexe.sortie.println("Methode est Realisable a ecrire");
		else
			Simplexe.sortie.println("Methode n'est pas Realisable a ecrire");
		// A modifier
		return flag; 
	}
	...
	// Deux lignes suivantes a supprimer
		...
		double [][]DD=new double[nbBase+1][nbHorsBase+1];
		
		DD[iS][jE]=1/D[iS][jE];
		
		for(int i=0;i<=nbBase;i++){
			if(i!=iS)
				DD[i][jE]=D[i][jE]/D[iS][jE];
			for(int j=0;j<=nbHorsBase;j++){
				if(j!=jE){
					DD[i][j]=D[i][j]-D[i][jE]*D[iS][j]/D[iS][jE];
				}
			}
		}
		...
		for(int j=0;j<=nbHorsBase;j++){
			if(j!=jE)
				DD[iS][j]=D[iS][j]/(-D[iS][jE]);
		}
		D=DD;
		int temp=varHorsBase[jE];
		varHorsBase[jE]=varBase[iS];
		varBase[iS]=temp;
