...
//		incomplet = true;
		...
		for(int i=1;i<=nbVarBase;i++){
			if(dict[i][jE]<0){
				double temp = - (dict[i][0]/dict[i][jE]);
				if(temp<sort){
					sort=temp;
					iS=i;
				}
...
//		incomplet = true;
		...
		for(int j=0;j<=nbVarHorsBase;j++){
			if(j!=jE){
				dict[iS][j]= -(dict[iS][j]/dict[iS][jE]);
			}			
		}
		
		dict[iS][jE]=1/dict[iS][jE];
		
		for(int i=0;i<=nbVarBase;i++){
			if(i!=iS){
				for(int j=0;j<=nbVarHorsBase;j++){
					if(j!=jE){
						dict[i][j]=dict[i][j]+dict[i][jE]*dict[iS][j];
					}
		...
		for(int i=1;i<=nbVarHorsBase;i++){
			if (dict[0][i]>0){
				...
				for(int j=1;j<=nbVarBase;j++){					
					if(dict[j][i]<0){						
						if(temp==0){
							temp=-(dict[0][i]*dict[j][0]/dict[j][i]);
						}else{
							double temp1= -(dict[0][i]*dict[j][0]/dict[j][i]);
							if(temp<temp1){
								temp=temp1;
							}
						}
...
//		incomplet = true;
		...
		for(int i=1;i<=nbVarHorsBase;i++){
			if (dict[0][i]>entre){
				...
				if(dict[0][i]==entre){
					if(tabVarHorsBase[i]<tabVarHorsBase[iE]){
						iE= i;	
					}
