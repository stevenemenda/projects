...
public class Descente extends Observable implements Runnable  {
	...
	private void directionASuivreSiInterieur() {
		Couple temp = problemeTraite.gradientf(pointCourant);
		temp = temp.mult(-1);
		if(temp.norme()< this.seuil){
			this.finie = true;
		}else{
			directionASuivre = temp;
		}				
	}
	...
	private void directionASuivreSiBord(Contrainte c) {
		Couple temp_gf = problemeTraite.gradientf(pointCourant);
		Couple ctemp = c.getGradient();//directionASuivre
		
		double ps = temp_gf.produitScalaire(ctemp);	
		Couple u = c.getBordUnitaire();
		if(temp_gf.norme()<this.seuil){
			this.finie = true;
		}else if(ps>0){
			directionASuivre = temp_gf.mult(-1);
			
		}else if(ps<0){
			if(temp_gf.estPerpendiculaire(u)){
				this.finie = true;
			}else{
				double du = temp_gf.produitScalaire(u);	
				if(du<0){
					directionASuivre = u;
				}else{
					directionASuivre = u.mult(-1);
				}
			}
	...
	* Si la descente n'est pas finie, met la prochaine direction dans l'attribut this.directionASuivre  
	...
	private void directionASuivreSiCoin(Contrainte[] coin) {
		...
		if(temp_gf.norme()<this.seuil){
			...
			Couple ncoin1= coin[1].getGradient().mult(-1);	
			Couple ncoin2= coin[2].getGradient().mult(-1);
			Couple ucoin1= new Couple();	
			Couple ucoin2= new Couple();
			Couple temp = coin[1].getBordUnitaire();
			double ps = temp.produitScalaire(ncoin2);
			if(ps<0){
				ucoin1= temp.mult(-1);
			}else{
				ucoin1= temp;
			}
			ps = temp.produitScalaire(ncoin1);
			if(ps<0){
				ucoin2= temp.mult(-1);
			}else{
				ucoin2= temp;
			}
			double ps1= ucoin1.produitScalaire(temp_gf);
			double ps2= ucoin2.produitScalaire(temp_gf);
			double fn1= ncoin1.produitScalaire(temp_gf);
			double fn2= ncoin2.produitScalaire(temp_gf);
			if((fn2<0)&&(fn1<0)){
				directionASuivre = temp_gf.mult(-1);
				...
				if((ps1>=0)&&(ps2>=0)){
					...
					directionASuivre = ucoin1;
				}else{
					directionASuivre = ucoin2;
				}
	...
	public Couple dichotomie(Couple pointDepart, Couple dir, double t1) {	
		...
		double t = problemeTraite.gPrime(pointDepart, dir, t1);
		Couple p= dir.mult(t).ajoute(pointDepart);
		Couple gf= problemeTraite.gradientf(p);
		// La valeur de retour est a modifier
